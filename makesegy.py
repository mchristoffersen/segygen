# Python 3.7.0
# Just needs numpy and pandas, struct, sys, os, multiprocessing are builtin
...
def gensegy(rgram, geom, sim, out):
    ## Assumption ##
    spt = 3600  # samples per trace
    ## ## ## ## ## ##

    # Load nav
    nav = pd.read_csv(geom, header=None)
    ...
    # Load data
    data = np.fromfile(rgram, dtype="float32")
    ...
    # Load sim (if provided)
    if(sim is not None):
        sims = np.fromfile(sim, dtype=np.float32)

        # Always 3600 samples per frame, two cluttergrams (left and right)
        # in each file
        nframe = len(sims) // 3600 // 2

        lsim = sims[: len(sims) // 2].reshape((3600, nframe))
        rsim = sims[len(sims) // 2 :].reshape((3600, nframe))
        combo_sim = lsim + rsim
        combo_sim = np.transpose(combo_sim).copy(order="C")
    ...
    with open(out + "_rgram.sgy", "wb") as f:
        ### Write 3200 byte text header
        f.write(build_txthead(out.split('/')[-1]))
        ...
        for i in range(ntrace):
            ...
            f.write(data[i, :].astype(">f", order="C").tobytes(order="C"))

    if(sim is not None):
        with open(out + "_sim.sgy", "wb") as f:
            ### Write 3200 byte text header
            f.write(build_txthead(out.split('/')[-1]))
            ...
            for i in range(ntrace):
                # print(int(lat[i]), int(lon[i]))
                f.write(build_trchead(spt, i + 1, int(lat[i]), int(lon[i])))
                f.write(combo_sim[i, :].astype(">f", order="C").tobytes(order="C"))
