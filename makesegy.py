...
def main():
  ...
  datapfix = "/zippy/MARS/orig/supl/SHARAD/PDS/DATA/RGRAM/"
  navpfix = "/zippy/MARS/orig/supl/SHARAD/PDS/DATA/GEOM/"
  outpfix = "/zippy/MARS/targ/supl/SHARAD/segy/"
  paths = []
  for id in ids:
    insubdir = id[0:6] + "XX/"
    datafile = id + "_RGRAM.IMG"
    navfile = id + "_GEOM.TAB"
    outfile = id + ".sgy"
    paths.append((datapfix+insubdir+datafile,navpfix+insubdir+navfile,outpfix+outfile))
  ...
  for i in range(len(paths)):
    if(not os.path.isfile(paths[i][0])):
      ...
      continue
    if(not os.path.isfile(paths[i][1])):
      navdne.append(ids[i])
      continue
    #if(os.path.isfile(paths[i][2])):
    #  sgyexist.append(ids[i])
    #  continue
  ...
  if(len(navdne) > 0):
    print("")
    print("Some nav files could not be found:")
    for i in navdne:
      print(i)
...
def gensegy(paths):
  ...
  obsname = data_path.split('/')[-1].replace("_RGRAM.IMG","")

  # Load nav
  nav = pd.read_csv(nav_path, header=None)
